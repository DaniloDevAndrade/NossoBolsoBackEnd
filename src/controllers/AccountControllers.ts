...
import { createUserSession } from "../services/authSession"
import { ChangePasswordSchema, UpdateProfileSchema } from "./schemas/AccountRequestsSchema"
...
export class AccountControllers {
  ...
  me: Handler = async (req, res, next) => {
    try {
      ...
      let partner: { id: string; name: string; email: string; phone: string } | null = null
  ...
  changePassword: Handler = async (req, res, next) => {
    try {
      ...
      const samePassword = compareSync(newPassword, user.password)
      if (samePassword) {
        return res
          .status(400)
  ...
  destroyAccount: Handler = async (req, res, next) => {
    try {
      const authUserId = (req as AuthedRequest).userId
